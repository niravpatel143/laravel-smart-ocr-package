...
class OpenAIVisionDriver implements OCRDriver
{
    protected array $config;
    protected CurlClient $http;

    private const API_URL  = 'https://api.openai.com/v1/chat/completions';
    private const MAX_BYTES = 20 * 1024 * 1024; // 20 MB — OpenAI limit
    ...
    public function extract($document, array $options = []): array
    {
        ...
        $dataUrl   = $this->readImageAsDataUrl($document);
    ...
    {
        $options['prompt'] = 'Read any barcode or QR code values. Return only decoded values, one per line.';
        $result = $this->extract($document, $options);
    ...
    private function textPrompt(?string $language): string
    {
        ...
        return "Extract ALL text exactly as it appears. Preserve layout and line breaks. Return only the text, no commentary.{$lang}";
    }

    private function readImageAsDataUrl(string $path): string
    {
        if (!file_exists($path)) {
        ...
        if (!isset($mimeToExt[$detected])) {
            throw new OCRException(
                "OpenAIVisionDriver supports: " . implode(', ', array_unique(array_values(self::MIME_MAP))) .
        ...
        if ($size > self::MAX_BYTES) {
            throw new OCRException(sprintf(
                "Image too large for OpenAI Vision (max 20 MB). File is %.1f MB.", $size / 1024 / 1024
            ));
        }

        $base64 = base64_encode(file_get_contents($path));

        return "data:{$detected};base64,{$base64}";
    }
