...
// Read any document
$result = SmartOCR::extract('invoice.pdf');
...
$result = SmartOCR::extractWithTemplate('new-invoice.pdf', $template->id);
```
...
```bash
php artisan smart-ocr:create-template 'Invoice Template' invoice --interactive
```

Process single documents:
```bash
php artisan smart-ocr:process document.pdf --ai-cleanup --save
```
...
```php
$result = SmartOCR::extract('spanish-invoice.pdf', [
...
$result = SmartOCR::extract('confidential.pdf'); // 100% private
```
...
$result = SmartOCR::extract(request()->file('invoice'));
...
// Configure in config/smart-ocr.php
'workflows' => [
    'invoice' => [
        'options' => [
            'use_ai_cleanup' => true,
